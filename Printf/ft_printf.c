...
/*   Updated: 2022/07/01 16:46:24 by guvargas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check(va_list params, char const *str, int i)
{
	...
	if (str[i] == 'c')
		...
		printed_chars += put_pointer((unsigned long int)va_arg(params, void *), 1);
	else if (str[i] == 'd' || str[i] == 'i')
		printed_chars += put_num(va_arg(params, int));
	else if (str[i] == 'u')
		printed_chars += put_u_num(va_arg(params, unsigned int));
	else if (str[i] == 'x')		
		printed_chars += put_hex(va_arg(params, unsigned int),'L');
	else if (str[i] == 'X')		
		printed_chars += put_hex(va_arg(params, unsigned int),'U');
	else if (str[i] == '%')
